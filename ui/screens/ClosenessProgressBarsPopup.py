...
class ClosenessProgressBarsPopup(Popup):
    def __init__(self, game_handler):
        super().__init__()
        self.game_handler = game_handler
        self.border_length = min(self.get_width() / 8, self.get_height() / 8)
        self.title = Label("title", "Apocalyptic Closeness", (0, -self.get_height() / 2 + self.border_length, 400, 50), 30, screen_size=self.screen_size)
        ...
        for i in range(0, num_apocalypses):
            ...
            self.apocalypse_bars[apocalypse_id] = ProgressBar(str(i), self.game_handler.apocalypse_handler.apocalypses[i].closeness,
                                                              (255, 100, 0), (0, 0, 0), (0, y, self.get_width() / 2, 0.5 * (end_y - start_y) / num_apocalypses),
    ...
    def receive_next_values(self, next_values):
        for apocalypse in self.game_handler.apocalypse_handler.apocalypses:
            self.apocalypse_bars[apocalypse.id].set_percent(apocalypse.closeness)
